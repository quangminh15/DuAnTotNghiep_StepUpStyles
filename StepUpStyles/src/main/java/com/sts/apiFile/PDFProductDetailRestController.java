...
@Controller
public class PDFProductDetailRestController {

    @PostMapping("/productdetail-pdf")
    @ResponseBody
    public void exportPdf(HttpServletResponse response, @RequestBody List<ProductDetail> productDetails)
            throws IOException, DocumentException {
        // Thiết lập response để trình duyệt hiểu rằng sẽ nhận tệp PDF
        response.setContentType("application/pdf");
        response.setHeader("Content-Disposition", "attachment; filename=DSProductDetail.pdf");
        ...
        PdfPTable table = new PdfPTable(6); // Số cột tùy thuộc vào số trường thông tin bạn muốn xuất
        ...
        PdfPCell productTitleCell = new PdfPCell(new Paragraph("Tên sản phẩm", font));
        productTitleCell.setHorizontalAlignment(Element.ALIGN_CENTER);
        table.addCell(productTitleCell);

        PdfPCell colorTitleCell = new PdfPCell(new Paragraph("Màu sắc", font));
        colorTitleCell.setHorizontalAlignment(Element.ALIGN_CENTER);
        table.addCell(colorTitleCell);

        PdfPCell sizeTitleCell = new PdfPCell(new Paragraph("Size", font));
        sizeTitleCell.setHorizontalAlignment(Element.ALIGN_CENTER);
        table.addCell(sizeTitleCell);

        PdfPCell quantityTitleCell = new PdfPCell(new Paragraph("Số lượng", font));
        ...
        float rowHeight = 30f;
        // Thêm nội dung vào tài liệu PDF từ danh sách sản phẩm chi tiết
        for (ProductDetail item : productDetails) {
            font = new Font(baseFont);

            float[] columnWidths = { 50f, 140f, 140f, 140f, 140f, 140f };
            table.setWidths(columnWidths);

            PdfPCell cell1 = new PdfPCell(new Paragraph(String.valueOf(item.getProductDetailID()), font));
            ...
            PdfPCell cell3 = new PdfPCell(new Paragraph(item.getColor().getColorName(), font));
            cell3.setHorizontalAlignment(Element.ALIGN_CENTER);
            cell3.setFixedHeight(rowHeight);
            table.addCell(cell3);

            PdfPCell cell4 = new PdfPCell(new Paragraph(item.getSize().getSizeNumber().toString(), font));
            cell4.setHorizontalAlignment(Element.ALIGN_CENTER);
            cell4.setFixedHeight(rowHeight);
            table.addCell(cell4);

            PdfPCell cell5 = new PdfPCell(new Paragraph(String.valueOf(item.getQuantity()), font));
        ...
        Chunk idChunk = new Chunk("Danh sách sản phẩm chi tiết", vietnameseFont);
