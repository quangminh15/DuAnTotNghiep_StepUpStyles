...
@Controller
public class ExcelProductImageRestController {

    @PostMapping("/export-excelProductImage")
    @ResponseBody
    public void exportExcel(HttpServletResponse response, @RequestBody List<ProductImage> productImages)
            throws IOException {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setHeader("Content-Disposition", "attachment; filename=ProductImageStepUpStyle.xlsx");

        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("Danh sách hình ảnh");
        ...
        // Tạo một ô để đặt giá trị "Danh sách hình ảnh"
        Cell detailCell = suppRow.createCell(1);
        detailCell.setCellValue("Danh sách hình ảnh");
        ...
        String[] headers = { "ID", "Đường dẫn ảnh", "Ngày điều chỉnh", "Tên sản phẩm" };
        ...
        for (ProductImage item : productImages) {
            ...
            row.createCell(0).setCellValue(item.getProductImageID());
            row.createCell(1).setCellValue(item.getImagePath());
