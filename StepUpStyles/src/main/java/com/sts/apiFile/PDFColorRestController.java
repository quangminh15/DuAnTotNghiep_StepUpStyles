...
@Controller
public class PDFColorRestController {

    @PostMapping("/color-pdf")
    @ResponseBody
    public void exportPdf(HttpServletResponse response, @RequestBody List<Color> colors)
            throws IOException, DocumentException {
        // Thiết lập response để trình duyệt hiểu rằng sẽ nhận tệp PDF
        response.setContentType("application/pdf");
        response.setHeader("Content-Disposition", "attachment; filename=DSColor.pdf");
        ...
        PdfPCell colorNameTitleCell = new PdfPCell(new Paragraph("Tên màu sắc", font));
        colorNameTitleCell.setHorizontalAlignment(Element.ALIGN_CENTER);
        table.addCell(colorNameTitleCell);
        ...
        float rowHeight = 30f;
        // Thêm nội dung vào tài liệu PDF từ danh sách màu sắc
        for (Color item : colors) {
            ...
            PdfPCell cell1 = new PdfPCell(new Paragraph(item.getColorID().toString(), font));
            ...
            PdfPCell cell3 = new PdfPCell(new Paragraph(item.getColorName(), font));
        ...
        Chunk idChunk = new Chunk("Danh sách màu của giày", vietnameseFont);
