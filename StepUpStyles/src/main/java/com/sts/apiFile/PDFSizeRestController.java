...
public class PDFSizeRestController {
    ...
    public void exportPdf(HttpServletResponse response, @RequestBody List<Size> sizes)
        ...
        PdfPTable table = new PdfPTable(5); // Số cột tùy thuộc vào số trường thông tin của đối tượng Size

        // Đặt tiêu đề cho từng cột
        table.addCell(new PdfPCell(new Paragraph("ID", font)));
        table.addCell(new PdfPCell(new Paragraph("Hoạt động", font)));
        table.addCell(new PdfPCell(new Paragraph("Số lượng", font)));
        table.addCell(new PdfPCell(new Paragraph("Ngày điều chỉnh", font)));
        ...
        for (Size item : sizes) {
            ...
            float[] columnWidths = { 40f, 80f, 60f, 60f, 90f }; // Thiết lập chiều rộng cho từng cột
