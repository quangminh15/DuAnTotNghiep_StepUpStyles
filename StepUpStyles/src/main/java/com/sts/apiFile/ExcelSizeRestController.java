...
public class ExcelSizeRestController {
    ...
    public void exportExcel(HttpServletResponse response, @RequestBody List<Size> sizes)
        ...
        String[] headers = { "Tên size", "Hoạt động", "Đã xóa", "Số lượng", "Ngày sửa đổi" };
        ...
        for (Size item : sizes) {
            ...
            row.createCell(0).setCellValue(item.getSizeNumber());
            row.createCell(1).setCellValue(item.getActivities());
            row.createCell(2).setCellValue(item.getDeleted());
            row.createCell(3).setCellValue(item.getSizeNumber());
            row.createCell(4).setCellValue(item.getModifyDate().toString());
