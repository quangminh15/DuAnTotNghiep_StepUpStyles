...
public class ExcelCategoryRestController {
    ...
    @PostMapping("/export-excelCategory")
    @ResponseBody
    public void exportExcel(HttpServletResponse response, @RequestBody List<Category> categories)
            throws IOException {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setHeader("Content-Disposition", "attachment; filename=CategoryStepUpStyle.xlsx");

        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.createSheet("Danh sách danh mục sản phẩm");
        ...
        // Tạo một ô để đặt giá trị "Danh sách danh mục"
        Cell detailCell = suppRow.createCell(1);
        detailCell.setCellValue("Danh sách danh mục");
        ...
        String[] headers = { "ID", "Hoạt động", "Đường dẫn ảnh", "Ngày điều chỉnh", "Tên danh mục" };
        ...
        for (Category category : categories) {
            ...
            String activityStatus = category.getActivities() ? "Đang hoạt động" : "Không hoạt động";
            row.createCell(1).setCellValue(activityStatus);

            row.createCell(2).setCellValue(category.getCategoryImage());

            Cell dateCell = row.createCell(3);
            if (category.getModifyDate() != null) {
                SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
                dateCell.setCellValue(dateFormat.format(category.getModifyDate()));
            }

            row.createCell(4).setCellValue(category.getCategoryName());
