...
public class PDFCategoryRestController {
	...
	@PostMapping("/category-pdf")
	@ResponseBody
	public void exportPdf(HttpServletResponse response, @RequestBody List<Category> categories)
			throws IOException, DocumentException {
		response.setContentType("application/pdf");
		response.setHeader("Content-Disposition", "attachment; filename=DSCategory.pdf");
		...
		PdfPCell nameTitleCell = new PdfPCell(new Phrase("Tên danh mục", font));
		nameTitleCell.setHorizontalAlignment(Element.ALIGN_CENTER);
		table.addCell(nameTitleCell);

		PdfPCell imageTitleCell = new PdfPCell(new Phrase("Ảnh danh mục", font));
		...
		for (Category item : categories) {
			...
			PdfPCell cell1 = new PdfPCell(new Phrase(String.valueOf(item.getCategoryID()), font));
			cell1.setHorizontalAlignment(Element.ALIGN_CENTER);
			cell1.setFixedHeight(rowHeight);
			table.addCell(cell1);

			PdfPCell cell2 = new PdfPCell(new Phrase(item.getCategoryName(), font));
			cell2.setHorizontalAlignment(Element.ALIGN_LEFT);
			cell2.setFixedHeight(rowHeight);
			table.addCell(cell2);

			PdfPCell cell3 = new PdfPCell(new Phrase(item.getCategoryImage(), font));
		...
		titleParagraph.add(new Phrase("DANH SÁCH DANH MỤC SẢN PHẨM", font));
